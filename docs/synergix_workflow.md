...
| 8 | SV9317 ("requires schedules to be set") when Submit is clicked before the schedule attaches in-view; also an intermittent, self-resolving slow calendar render mistaken for a missing newEventButton | **Fixed** (`59e1e16`): `_submit_and_recover()` detects SV9317 (appears top-right, sometimes off-screen) and retries once via a Schedule Calendar re-click, matching the exact recovery the user demonstrated live by hand |
