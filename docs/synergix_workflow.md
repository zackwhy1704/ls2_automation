...
| 8 | SV9317 ("requires schedules to be set") when Submit is clicked before the schedule attaches in-view | **Fixed** (`59e1e16`): `_submit_and_recover()` detects SV9317 (appears top-right, sometimes off-screen) and retries once via a Schedule Calendar re-click, matching the exact recovery the user demonstrated live by hand |
| 9 | "No newEventButton found" -- a REAL date collision (not a UI lag as first misdiagnosed): every synthetic test WO shares the same hardcoded `ASSIGNED_WORK_TEAM`, and the old `% 300` date-offset formula produced real collisions across the session's ~27 test WOs | **Fixed for real** (`228534b`): widened to `% 3650` and reused the better-distributed `_char_sum` from the WO-number derivation, confirmed zero collisions across all of today's suffixes |
